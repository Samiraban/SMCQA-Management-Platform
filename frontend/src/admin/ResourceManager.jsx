...
function ResourceManager({
  ...
  /* ---------------------------------------------------------
     FILE UPLOAD
     Single-value fields: "image" / "icon"
     --------------------------------------------------------- */
  async function handleFileUpload(field, fileList) {
    ...
    try {
      ...
      setError(
        uploadError.message || "Unable to use that image."
      );
    } finally {
      setUploadingKey("");
    }
  ...
  async function handleGalleryUpload(field, fileList) {
    ...
    try {
      ...
      setError(
        uploadError.message ||
          "Unable to use one of those images."
      );
    } finally {
      setUploadingKey("");
    }
                ...
                <div key={field.key}>
                  ...
                  {field.type === "textarea" ? (
                    ...
                    <>
                      {/* =================================================
                          ICON INPUT
                          No fixed dropdown / datalist.
                          User can type ANY Lucide icon name.
                         ================================================= */}
                      <div className="admin-image-field">
                        <input
                          type="text"
                          placeholder={
                            field.placeholder ||
                            "e.g. Hotel, Plane, ChefHat, Building2"
                          }
                          value={
                            isImageSource(
                              editing[field.key]
